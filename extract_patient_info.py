...
def extract_patient_info(data_dir):
    patient_info_list = []
                    ...
                    for line in f:
                        if line.startswith('Patient ID: '):
                            ...
                            try:
                                ...
                                lbp = int(bp[0]) if bp != 'n/a' and len(bp) > 1 else -1
                                hbp = int(bp[1]) if bp != 'n/a' and len(bp) > 1 else -1
...
def main():
    ...
    patient_info_list = extract_patient_info(data_dir)
    save_to_csv(patient_info_list, output_file)
